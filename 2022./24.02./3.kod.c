...
int printVehicle(VehicleP vehicle);
int printList2(PersonP head);
...
int main() {
	...
	printf("\n\n\nLista vozila kojima je istekla registracija : \n");
	printList2(head);
...
int printList2(PersonP head) {
	...
	while (temp) {
		...
		while (vehicle) {
			if (vehicle->date.year < 2025) {
				printVehicle(vehicle);
				found = 1;
				vehicle = vehicle->next;																		
			}
			else {
				if ((vehicle->date.year == 2025) && (vehicle->date.month < 8)) {
					printVehicle(vehicle);
					found = 1;
					vehicle = vehicle->next;
				}
				else {
					if ((vehicle->date.year == 2025) && (vehicle->date.month == 8) && (vehicle->date.day < 28)) {
						printVehicle(vehicle);
						found = 1;
					}
					vehicle = vehicle->next;
		...
		if (!found) {
			printf("Nijednom autu nije istekla registracija!\n");
		}
		temp = temp->next;
	}
	return EXIT_SUCCESS;
