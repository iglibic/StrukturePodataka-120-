...
//Ovaj se može puno bolje uraditi
...
int moveToTheFront(EmployeeP head, EmployeeP temp);
int findHighestSalary(EmployeeP head);
...
int main() {
    ...
    moveHighestSalary(head);
    moveSecondHighestSalary(head);
    moveThirdHighestSalary(head);
    printf("\n");
    printList(head->next);
...
int moveHighestSalary(EmployeeP head) {
    EmployeeP prev = head;
    EmployeeP curr = head->next;
    int highest = 0;
    while (curr) {
        if (curr->salary > highest) {
            highest = curr->salary;
        }
        prev = curr;
        curr = curr->next;
    ...
    EmployeeP prev1 = head;
    EmployeeP curr1 = head->next;
    while (curr1) {
        if (highest == curr1->salary) {
...
int moveSecondHighestSalary(EmployeeP head) {
    EmployeeP prev = head->next;
    EmployeeP curr = head->next->next;
    int highest = 0;
    while (curr) {
        if (curr->salary > highest) {
            highest = curr->salary;
...
int moveThirdHighestSalary(EmployeeP head) {
    EmployeeP prev = head->next->next;
    EmployeeP curr = head->next->next->next;
    int highest = 0;
    while (curr) {
        if (curr->salary > highest) {
            highest = curr->salary;
